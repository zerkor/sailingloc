...
const Header = () => {
  ...
  const navLinkClass = ({ isActive }) =>
    `relative text-sm font-medium transition-colors duration-200 after:absolute after:-bottom-1 after:left-0 after:right-0 after:h-[1.5px] after:bg-cyan-500 after:transition-transform after:duration-300 ${
      isActive ? 'text-white after:scale-x-100' : 'text-white/60 hover:text-white after:scale-x-0 hover:after:scale-x-100'
    }`;

  return (
    <header className="sticky top-0 z-40 border-b border-white/[0.08]" style={{ background: 'rgba(7,25,46,0.97)', backdropFilter: 'blur(20px)' }}>
      <nav className="container-max px-4 sm:px-6 lg:px-14">
        <div className="flex items-center justify-between h-[76px]">
          ...
          <div className="hidden lg:flex items-center gap-7">
            ...
            {user?.role === 'owner' && <NavLink to="/owner/dashboard" className={navLinkClass}>Mon espace</NavLink>}
            {user?.role === 'admin' && <NavLink to="/admin/dashboard" className={navLinkClass}>Administration</NavLink>}
          </div>

          <div className="hidden lg:flex items-center gap-3">
            {user ? (
              <div className="relative">
                <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2 text-sm font-medium text-white/80 hover:text-white bg-white/10 hover:bg-white/15 border border-white/15 rounded-full px-4 py-2 transition-all">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: '#00C6E0', color: '#07192E' }}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl border border-navy-900/10 py-1.5 z-50" style={{ boxShadow: '0 12px 48px rgba(7,25,46,0.14)' }}>
                    ...
                    <button onClick={handleLogout} className="block w-full text-left px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 transition-colors">Déconnexion</button>
                  </div>
                )}
              </div>
            ) : (
              <>
                <Link to="/login" className="text-sm font-medium text-white/70 hover:text-white border border-white/20 hover:border-white/50 px-5 py-2 rounded-full transition-all duration-200">Connexion</Link>
                <Link to="/register" className="text-sm font-bold px-5 py-2 rounded-full transition-all duration-200 hover:opacity-90" style={{ background: '#00C6E0', color: '#07192E' }}>S'inscrire</Link>
              </>
            )}
          </div>

          <button className="lg:hidden flex flex-col gap-[5px] p-2 rounded-lg" onClick={() => setMenuOpen(!menuOpen)} aria-label="Menu">
        ...
        {menuOpen && (
          <div className="lg:hidden pb-5 border-t border-white/10 pt-4 space-y-0.5">
            {navLinks.map(link => (
              <Link key={link.to} to={link.to} className="block px-3 py-3 text-sm font-medium text-white/80 hover:text-white rounded-xl hover:bg-white/5" onClick={() => setMenuOpen(false)}>
                {link.label}
              </Link>
            ))}
