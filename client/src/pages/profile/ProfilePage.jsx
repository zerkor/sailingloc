...
const ProfilePage = () => {
  ...
  return (
    <div className="container-max section-padding max-w-2xl" style={{ minHeight: '80vh' }}>
      <h1 className="mb-8" style={{ fontFamily: "'Playfair Display', serif", fontSize: 32, fontWeight: 800, color: '#07192E' }}>
        Mon profil
      </h1>

      {/* Avatar card */}
      <div className="bg-white rounded-2xl p-6 mb-5 flex items-center gap-5" style={{ boxShadow: '0 4px 24px rgba(7,25,46,0.08)' }}>
        <div
        ...
        <div>
          <p className="text-lg font-bold" style={{ color: '#07192E' }}>{user?.firstName} {user?.lastName}</p>
          <p className="text-sm" style={{ color: '#8896A8' }}>{user?.email}</p>
          <span
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input id="firstName" type="text" name="firstName" value={form.firstName} onChange={handleChange} className="input-field" required />
            </div>
            <div>
              <label htmlFor="lastName" className="block text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#3D4D61' }}>Nom</label>
              <input id="lastName" type="text" name="lastName" value={form.lastName} onChange={handleChange} className="input-field" required />
            </div>
          </div>
          <div>
            <label htmlFor="phone" className="block text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#3D4D61' }}>Téléphone</label>
            <input id="phone" type="tel" name="phone" value={form.phone} onChange={handleChange} className="input-field" placeholder="+33 6 00 00 00 00" />
          </div>
      ...
      {/* Danger zone */}
      <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 24px rgba(7,25,46,0.08)', border: '1px solid rgba(239,68,68,0.2)' }}>
        <h2 className="font-bold text-base mb-2" style={{ color: '#dc2626' }}>Zone dangereuse</h2>
