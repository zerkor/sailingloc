...
const ContactPage = () => {
  ...
  return (
    <div style={{ background: '#F7F5F2', minHeight: '100vh' }}>
      ...
      {/* ── Hero banner ── */}
      <div className="py-16 px-4 sm:px-6 lg:px-14" style={{ background: '#07192E' }}>
        <div className="container-max">
          <span className="sec-eyebrow" style={{ color: '#00C6E0' }}>Contact</span>
          <h1
            className="mt-1"
            style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(28px,4vw,48px)', fontWeight: 800, color: '#fff', lineHeight: 1.1 }}
          >
      ...
      <div className="container-max section-padding">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          ...
          {/* ── Left: form ── */}
          <div className="bg-white rounded-3xl p-8 sm:p-10" style={{ boxShadow: '0 4px 24px rgba(7,25,46,0.08)' }}>
            <h2
              className="mb-6"
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              {/* Name */}
              <div>
                <label htmlFor="ct-name" className={LABEL_CLS} style={{ color: '#3D4D61' }}>Nom complet *</label>
                <input
              ...
              {/* Email */}
              <div>
                <label htmlFor="ct-email" className={LABEL_CLS} style={{ color: '#3D4D61' }}>Email *</label>
                <input
                  id="ct-email" name="email" type="email"
                  value={form.email} onChange={handleChange}
                  className="input-field" placeholder="jean.dupont@exemple.fr"
                  style={fieldStyle('email')}
                />
                {errors.email && <p className="mt-1 text-xs font-medium" style={{ color: '#dc2626' }}>{errors.email}</p>}
              </div>

              {/* Subject */}
              <div>
                <label htmlFor="ct-subject" className={LABEL_CLS} style={{ color: '#3D4D61' }}>Sujet *</label>
                <select
                  id="ct-subject" name="subject"
                ...
                {errors.subject && <p className="mt-1 text-xs font-medium" style={{ color: '#dc2626' }}>{errors.subject}</p>}
              </div>

              {/* Message */}
              <div>
                <label htmlFor="ct-message" className={LABEL_CLS} style={{ color: '#3D4D61' }}>Message *</label>
                <textarea
                ...
                />
                {errors.message && <p className="mt-1 text-xs font-medium" style={{ color: '#dc2626' }}>{errors.message}</p>}
              </div>
