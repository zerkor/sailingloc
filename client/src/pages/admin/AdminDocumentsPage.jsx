...
const AdminDocumentsPage = () => {
  ...
  const review = async (id, nextStatus) => {
    const rejectionReason =
      nextStatus === 'rejected'
        ? (await askText('Motif du rejet ?', {
            title: 'Rejeter le document',
            placeholder: 'Document non conforme',
    ...
    try {
      ...
      toast(nextStatus === 'approved' ? 'Document approuvé.' : 'Document rejeté.', 'success');
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 800, color: '#07192E' }}>
          Documents <span style={{ fontSize: 18, color: '#8896A8' }}>({meta.total})</span>
        </h1>
        <select
          value={status}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: '#EDF1F5' }}>
                ...
                <th className="px-5 py-3 text-left">Uploaded date</th>
