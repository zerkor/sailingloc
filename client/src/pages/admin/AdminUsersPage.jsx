...
const AdminUsersPage = () => {
  ...
  const updateUser = async (id, payload) => {
    try {
      ...
      toast(err.response?.data?.message || 'Action refusée.', 'error');
    }
  };
  ...
  const changeRole = async (target, nextRole) => {
    ...
    if (
      !(await requestApproval(`Changer le rôle de ${target.email} en ${roleLabels[nextRole]} ?`, {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 800, color: '#07192E' }}>
            Utilisateurs <span style={{ fontSize: 18, color: '#8896A8' }}>({meta.total})</span>
          </h1>
          <p className="text-sm mt-1" style={{ color: '#64748B' }}>
            {meta.activeAdminCount} administrateur(s) actif(s)
          </p>
        </div>
      ...
      {error && (
        <div className="rounded-2xl p-4 text-sm font-semibold" style={{ background: '#fee2e2', color: '#991b1b' }}>
          {error}
        </div>
      )}
              ...
              {users.length === 0 ? (
                ...
                users.map((target) => {
                  ...
                  return (
                    <tr
                      key={target._id}
                      style={{ borderBottom: '1px solid rgba(7,25,46,0.06)', opacity: target.isActive ? 1 : 0.6 }}
                    >
                      ...
                      <td className="px-5 py-3">
                        <select
                          value={target.role}
                          disabled={dangerousDisabled && target.role === 'admin'}
                          onChange={(e) => changeRole(target, e.target.value)}
                          className="input-field text-xs min-w-[130px]"
                        >
                      ...
                      <td className="px-5 py-3 text-right">
                        {dangerousDisabled && (
                          <span className="inline-flex items-center gap-1 text-xs mr-2" style={{ color: '#b45309' }}>
                        ...
                        {target.isActive ? (
                          <button
                            ...
                            onClick={() => updateUser(target._id, { isActive: false })}
                          ...
                          <button
                            onClick={() => updateUser(target._id, { isActive: true })}
